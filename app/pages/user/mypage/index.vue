        ...
        <m-column>
            ...
            <m-form v-if="profileModel" title="プロフィール設定" bordered>
                ...
                <c-labeled-item label="好きな音楽の年代">
                    <c-dropdown :model.sync="profileModel.favoriteMusicAge"  :items="musicAgeItems" data-value="value" />
                </c-labeled-item>
                <c-labeled-item label="好きなアーティスト">
                    <c-text-input :model.sync="profileModel.favoriteArtist" />
                </c-labeled-item>
...
export default class PageMypage extends Vue {
    ...
    async saveProfileHandler() {
        ...
        try {
            ...
            await this.$axios.$put(`api/user/${this.profileModel.id}/update`, this.profileModel)
